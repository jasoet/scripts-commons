...
object GCloudSpec : Spek({
    describe("GCloud Config Helper") {
        val configDir = "/tmp/GcloudConfigDir"
        val credContent = "CREDSCONTENT"
        val encodedCreds = Base64.getEncoder().encodeToString(credContent.toByteArray())
        context("Initialize GCloud Helper") {
            val configPath = GCloud(encodedCreds, configDir)
